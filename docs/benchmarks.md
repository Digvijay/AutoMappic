...
description: Detailed performance analysis of AutoMappic v0.6.0, demonstrating 6.5x faster runtime execution and zero-allocation mapping compared to legacy .NET mappers.
---

# Benchmarks: Performance Analysis of AutoMappic v0.6.0
...
### Analysis
AutoMappic maintains **parity with hand-written manual mapping** (~29ns vs ~28ns) and is **6.5x faster** than AutoMapper. The v0.6.0 additions (ProjectTo, Hardened Incremental Pipeline) maintain this extreme performance profile while ensuring IDE stability.

## 3. Results: Collection Mapping (IMapper Interception)

v0.6.0 introduces full interception for `IMapper.Map<List<T1>, List<T2>>()` and advanced `ProjectTo` optimizations.
...
In v0.6.0, `MappingContext` remains a `readonly struct` for maximum performance on nested graphs. 
