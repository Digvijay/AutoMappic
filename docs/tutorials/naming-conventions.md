...
- `KebabCaseNamingConvention`: Standard `kebab-case` matching (e.g., `first-name`). (v0.5.0)
...
## Dictionary & Reader Conventions (v0.5.0)
...
In v0.5.0, the `PascalCase` and `CamelCase` conventions have been upgraded to handle acronyms (e.g., `CustomerID` → `customerId` or `customer-id`).
