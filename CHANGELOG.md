...
### Fixed
- **VS Code Code-Fix Fix**: Bundled missing MEF dependencies (`System.Composition`) into the NuGet package to ensure that IDE lightbulbs (for `AM0018` and `AM0015`) correctly fire in VS Code, OmniSharp, and .NET 10 environments.

### Improved
- **Smart-Match (AM0015)**: Lowered default threshold to `0.4` and implemented name normalization during fuzzy matching. The generator now correctly suggests mappings across different naming styles (snake_case vs PascalCase) and for different but mappable types (e.g. `DateTime` → `DateTimeOffset`).
