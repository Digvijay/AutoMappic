...
internal static class SourceEmitter
{
    public static (string HintName, string Source) EmitMappingClass(MappingModel model, IReadOnlyDictionary<string, MappingModel> mappings)
    {
        ...
        sb.AppendLine("#pragma warning disable CS8600, CS8601, CS8602, CS8603, CS8604 // Suppress nullability warnings in generated mapping logic");
        sb.AppendLine($"// EnableEntitySync={model.EnableEntitySync}");
        sb.AppendLine();
        ...
        if (!hasProjRuntimeFeatures && model.TypeConverterFullName == null && model.StaticConverterMethodFullName == null && string.IsNullOrEmpty(typeParamsStr))
        {
            sb.AppendLine();
    ...
    private static void EmitInPlaceBody(StringBuilder sb, MappingModel model, bool isAsync, IReadOnlyDictionary<string, MappingModel> mappings)
    {
        ...
        // AfterMap: wrap in local suppression if needed to avoid user-code warnings (e.g. CS8602) polluting the generator output
        if (!string.IsNullOrEmpty(model.AfterMapBody) || (isAsync && !string.IsNullOrEmpty(model.AfterMapAsyncBody)))
        {
        ...
        IReadOnlyDictionary<string, MappingModel> mappingsByKey)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>Internal compiler attribute for interceptors. This is an experimental Roslyn feature.</summary>");
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        public string FilePath { get; } = filePath;");
        sb.AppendLine("        public int Line { get; } = line;");
        sb.AppendLine("        public int Column { get; } = column;");
        sb.AppendLine("    }");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("#pragma warning disable CS9270 // Experimental interceptors warning");
        sb.AppendLine();
        sb.AppendLine("namespace AutoMappic.Generated");
        sb.AppendLine("{");
            ...
            else if (item.Kind == InterceptKind.ProjectTo)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (model == null)
                        {
                            shimSb.AppendLine("             throw new global::AutoMappic.AutoMappicException(\"Mapping not found for projection.\");");
                        }
                        else
                        {
                            ...
                            {
                                shimSb.Append($"             return (({fullSource})source).Select(source => new {item.DestinationTypeFullName}(");
                                shimSb.Append(string.Join(", ", targetCtor.Select(c => c.SourceExpression?.Replace(", context)", ")").Replace("(context)", "()").TrimEnd('!') + "!")));
                                shimSb.AppendLine(")");
                            }
                            else
                            {
    ...
    public static (string HintName, string Source) EmitRegistration(string assemblyName, ImmutableArray<string?> profiles, ImmutableArray<MappingModel> localMappings, ImmutableArray<string> referencedAssemblies, bool isEntryPoint)
    {
        ...
        foreach (var m in localMappings)
        {
            ...
            if (src.StartsWith("global::", StringComparison.Ordinal) && !src.EndsWith("::S", StringComparison.Ordinal) && !src.EndsWith("::D", StringComparison.Ordinal) &&
                ...
                !src.EndsWith(".S", StringComparison.Ordinal) && !src.EndsWith(".D", StringComparison.Ordinal) &&
        ...
        sb.AppendLine("namespace AutoMappic.Generated");
        sb.AppendLine("{");
        sb.AppendLine($"    /// <summary>Auto-generated registration marker for {sanitized}.</summary>");
        sb.AppendLine($"    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]");
